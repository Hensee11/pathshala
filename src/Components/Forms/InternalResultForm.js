...
const InternalResultForm = () => {
  ...
  const fetchInternal = async (e) => {
    ...
    e.preventDefault();
    try {
      const response = await axios.get("/internal/" + subject);
  ...
  const addInternalMark = async (e) => {
    e.preventDefault();
    const marks = { id, subject, marks: internal };
    try {
      const response = await axios.post("internal/" + subject, marks);
      ...
      if (err.response.status === 409) {
        try {
          const response = await axios.patch("internal/" + subject, marks);
          toast.success(response.data.message);
          setDisabled(true);
          setError("");
        } catch (err) {
